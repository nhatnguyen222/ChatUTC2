...
namespace chatUTC2.Hubs
{
    public class WebRTCHub : Hub
    {
        ...
        public async Task CreateRoom(string name)
        {
            ...
            {
                await Clients.Caller.SendAsync("error", "Lỗi khi tạo phòng.");
            }
        }
        ...
        public async Task NotifyRoomInfoAsync(bool notifyOnlyCaller)
        {
            ...
            var list = from room in roomInfos
                       select new
                       {
                           RoomId = room.RoomId,
                           Name = room.Name,
                           Button = "<button class=\"joinButton\">Vào</button>"
                       };
            var data = JsonConvert.SerializeObject(list);

            if (notifyOnlyCaller)
            {
                await Clients.Caller.SendAsync("capNhatPhong", data);
            }
            else
            {
                await Clients.All.SendAsync("capNhatPhong", data);
            }
        }
    }
